...
describe('CS142 Photo App API - ', function () {
    ...
    describe('test /user/advanced/list/', function(done) {
        var userList;
        var cs142Users = cs142models.userListModel();
        ...
        it('has an entry for each of the users', function (done) {
            async.each(cs142Users, function (realUser, callback) {
                ...
                assert(user, 'could not find user ' + realUser.first_name + ' ' + realUser.last_name);
                assert.strictEqual(_.countBy(userList, '_id')[user._id], 1, 'Multiple users with id:' + user._id);
                var photosOfUser = cs142models.photoOfUserModel(realUser._id);
                assert.strictEqual(user.photo_count, photosOfUser.length, 'wrong photo count. actual: ' + photosOfUser.length + ' got: ' + user.photo_count);
                var commentsOfUser = cs142models.commentOfUserModel(realUser._id);
                assert.strictEqual(user.comment_count, commentsOfUser.length, 'wrong comment count. actual: ' + commentsOfUser.length + ' got: ' + user.comment_count);
                callback();
            }, done);
        });
    ...
    describe('test /photosOfUser/:id?photoId=:photoId', function(done) {
        var userList;
        var cs142Users = cs142models.userListModel();
        ...
        it('can get photos of user with /photosOfUser/:id', function(done) {
            async.each(cs142Users, function (realUser, doneUserCallback) {
                var user = _.find(userList, {first_name: realUser.first_name, last_name: realUser.last_name});
                assert(user, 'user not found: ' + realUser.first_name + ' ' + realUser.last_name);
                var cs142PhotosOfUser = cs142models.photoOfUserModel(realUser._id);
                http.get({
                    hostname: host,
                    port: port,
                    path: '/photosOfUser/' + user._id,
                    headers: {'Cookie': authCookie}
                }, function (response) {
                    var responseBody = '';
                    response.on('data', function (chunk) {
                    ...
                    response.on('end', function () {
                        assert.strictEqual(response.statusCode, 200, 'HTTP response status code not OK');
                        var photoList = JSON.parse(responseBody);
                        photoLists[user._id] = photoList;
                        assert.strictEqual(cs142PhotosOfUser.length, photoList.length, 'not same length of photos');
                        doneUserCallback();
                    });
                });
            }, function(err) {
        ...
        it('can get a photo with photosOfUser/:id?photoId=:photoId', function(done) {
            async.each(cs142Users, function (realUser, doneUserCallback) {
                var user = _.find(userList, {first_name: realUser.first_name, last_name: realUser.last_name});
                assert(user, 'user not found: ' + realUser.first_name + ' ' + realUser.last_name);
                var cs142PhotosOfUser = cs142models.photoOfUserModel(realUser._id);
                var photoList = photoLists[user._id];
                async.each(cs142PhotosOfUser, function(realPhoto, donePhotoCallback) {
                    var photo = _.find(photoList, {file_name: realPhoto.file_name});
                    assert(photo, 'photo not found in photoList: ' + realPhoto.file_name + photoList);
                    var photos;
                    http.get({
                        hostname: host,
                        port: port,
                        path: '/photosOfUser/' + user._id + '?photoId=' + photo._id,
                        headers: {'Cookie': authCookie}
                    }, function (response) {
                        var responseBody = '';
                        response.on('data', function (chunk) {
                        ...
                        response.on('end', function () {
                            assert.strictEqual(response.statusCode, 200, 'HTTP response status code not OK');
                            photos = JSON.parse(responseBody);
                            assert(photos, 'could not find photo: ' + realPhoto.file_name)
                            assert.strictEqual(photos.length, 1, 'not 1 photo');
                            assert.strictEqual(photos[0]._id, photo._id, 'wrong id');
                            assert.strictEqual(realPhoto.file_name, photos[0].file_name, 'wrong file name');
                            donePhotoCallback();
                        });
                    });
                }, function(err) {
                    //done all photos for one user
                    doneUserCallback();
                });
            }, function(err) {
                //done with all users
